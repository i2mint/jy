...
def default_token_replacer(matched_token: str, token_class: str):
    ...
    """
    unique_suffix = str(uuid.uuid4()).replace('-', '')[:8]  # Taking the first 8 characters for brevity
    ...
    if token_class == "id" and matched_token.startswith('document.getElementById('):
        return 'document.getElementById("' + matched_token.split('"')[1] + '_' + unique_suffix + '")'
...
def _is_js_string(string):
    return "document.getElementById" in string or "function" in string or ".addEventListener" in string

def _is_css_string(string):
    return "{" in string and "}" in string and (":" in string or ";" in string)

_dflt_string_kind_rules = tuple({
    'html': _is_html_string, 'js': _is_js_string, 'css': _is_css_string 
}.items())

def _string_kind(
    string, string_kind_rules = _dflt_string_kind_rules
...
_dflt_replacer_for_kind = tuple({
    'html': replace_html_ids, 'js': replace_js_ids, 'css': replace_css_ids
}.items())
...
def test_replace_tokens():
    ...
    assert new_string != test_string  # The new string should be different due to the unique ID
